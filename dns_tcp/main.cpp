...
void parse_dns(unsigned char *payload, int length, const fabs_appif_header &hdr);
...
public:
    ...
    dns_reader() : m_dns_state(STATE_LEN) {}

    void push(ptr_fabs_bytes bytes) {
        m_buf.push_back(std::move(bytes));
    }

    void parse(const fabs_appif_header &hdr) {
        for (;;) {
            if (m_dns_state == STATE_LEN) {
                int n = read_bytes(m_buf, (char*)&m_buflen, sizeof(m_buflen));
                if (n != sizeof(m_buflen)) {
                    break;
                } else {
                    skip_bytes(m_buf, sizeof(m_buflen));
                    m_buflen = ntohs(m_buflen);
                    m_dns_state = STATE_BODY;
                }
            } else {
                unsigned char buf[65536];
                int n = read_bytes(m_buf, (char*)buf, m_buflen);
                if (n != m_buflen) {
                    break;
                } else {
                    parse_dns(buf, m_buflen, hdr);
                    skip_bytes(m_buf, m_buflen);
                    m_dns_state = STATE_LEN;
                }
            }
        }
...
private:
    std::deque<ptr_fabs_bytes> m_buf;
};
...
int
ns_print(ns_msg* ns_handle, int format, const fabs_appif_header &hdr)
{
    /*
    // flags
    ...
    */
    ...
    if (format == 1) {
       ...
       for (i=0; i<query_count; i++) {
        ...
        printf("\"src\":{");
        if (hdr.from == FROM_ADDR1) {
            printf("\"ip\":\"%s\",", buf1);
            printf("\"port\":%d", ntohs(hdr.l4_port1));
        } else {
            printf("\"ip\":\"%s\",", buf2);
            printf("\"port\":%d", ntohs(hdr.l4_port2));
        }
        printf("},");
...
void parse_dns(unsigned char *payload, int length, const fabs_appif_header &hdr)
{
    ...
    ns_print(&ns_handle, ns_format, hdr);
...
{
    int result;
    std::string uxpath = (char*)"/tmp/sf-tap/tcp/dns";
    ...
    std::map<fabs_id_dir, std::unique_ptr<dns_reader>> readers;
    ...
    for (;;) {
        ...
        fabs_id_dir id_dir;
        id_dir.m_id.set_appif_header(hdr);

        switch (hdr.event) {
        case 0: // CREATED
            id_dir.m_dir = FROM_ADDR1;
            readers[id_dir] = std::unique_ptr<dns_reader>(new dns_reader);
        ...
        case 2: // DATA
            ...
            auto it = readers.find(id_dir);

            if (hdr.len > 0) {
                ptr_fabs_bytes bytes(new fabs_bytes);
                bytes->alloc(hdr.len);

                int len = read(sock, bytes->get_head(), hdr.len);
                if (len <= 0) {
